...
def check_whole_page_html_for_credits(driver):
    ...
    """
    try:
        print("🔍 Performing quick search through entire page HTML for credit keywords...")
        ...
        print(f"   📄 Searching through {len(page_text)} characters of visible text and {len(page_html)} characters of HTML source")
        
        # Debug: Show a sample of the page text
        sample_text = page_text[:300] + "..." if len(page_text) > 300 else page_text
        print(f"   📝 Sample visible text: {sample_text}")
        ...
        for keyword in CREDIT_KEYWORDS:
            ...
            if matches_keyword_with_word_boundary(keyword, page_text):
                ...
                if keyword_index != -1:
                    ...
                    print(f"   ✅ Found '{keyword}' in visible page text at position {keyword_index}")
            ...
            # Also check in raw HTML source (in case it's in attributes, comments, etc.)
                ...
                if keyword_index != -1:
                    ...
                    print(f"   ✅ Found '{keyword}' in HTML source at position {keyword_index}")
                    print(f"      HTML Context: ...{context}...")
        
        print(f"   🔍 Checked {checked_keywords} keywords in page content")
        
        if found_keywords:
            print(f"✓ Found {len(found_keywords)} credit keywords in page HTML")
        else:
            print("✓ No credit keywords found in page HTML")
        
        return found_keywords, found_texts
        
    except Exception as e:
        print(f"⚠️ Error searching page HTML for credits: {e}")
        return [], []
